(() => {
  ...
  function SpectrogramViz(){
    ...
    return {
      controls:[
        {type:"play",label:()=>"Open the spectrogram lab",on:()=>{ window.location.href="/upload"; }}
      ],
      note:"three source modes: sweep · upload · microphone",
      hint:"The old in-page Start button has been replaced. Open the lab to choose Sweep, Upload, or Microphone.",
      mount(host){
        cv=el("canvas");
        host.appendChild(cv);
        loop();
    ...
    function draw(){
      ...
      ctx.fillText("Open the spectrogram lab",W/2,by+31*dpr);
      ctx.textAlign="left";
    }
